...
import { ChevronRight, Clock3, Coffee, HelpCircle, Home, LogOut, MapPin, Minus, Plus, ShoppingBag, Trash2, UserRound, Utensils } from 'lucide-react'
            ...
            <div className="grid gap-6 xl:grid-cols-[1.08fr_0.72fr]">
              <div className="space-y-5">
                ...
                {cartItems.length > 0 ? (
                  <section className="overflow-hidden rounded-[1.4rem] border border-[#dfddd2] bg-white shadow-[0_1px_0_rgba(18,61,50,0.04)]">
                    <div className="flex items-center justify-between border-b border-[#ece9df] px-4 py-4 sm:px-5">
                      <div className="flex items-center gap-2 text-[#123d32]">
                        ...
                        <h2 className="text-[1.02rem] font-bold">Keranjang Aktif</h2>
                      </div>
                    ...
                    <div className="divide-y divide-[#ece9df]">
                      {cartItems.map((item) => (
                        <article key={item.id} className="grid grid-cols-[72px_1fr] gap-3 px-4 py-3 sm:grid-cols-[92px_1fr] sm:px-5">
                          <img src={item.image} alt={item.name} className="h-[72px] w-[72px] rounded-[0.9rem] object-cover sm:h-[82px] sm:w-[82px]" />
                          ...
                          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
                            ...
                            <div className="flex items-center justify-between gap-4 sm:flex-col sm:items-end sm:justify-center">
                              <p className="text-[0.98rem] font-bold text-[#d64b3b]">Rp {item.price.toLocaleString('id-ID')}</p>
                    ...
                    <button type="button" className="flex w-full items-center justify-center gap-2 border-t border-[#ece9df] py-3 text-[0.82rem] font-semibold text-[#123d32] transition hover:bg-[#fbfaf5]">
                      Tambah Pesanan
                  ...
                  <section className="rounded-[1.4rem] border border-dashed border-[#cfd8cf] bg-white p-6 text-center shadow-[0_1px_0_rgba(18,61,50,0.04)] sm:p-8">
                    ...
                    <p className="mt-2 text-sm leading-relaxed text-[#69766c]">Tidak ada data dummy di halaman ini. Silakan pilih makanan dari beranda untuk menambahkan pesanan.</p>
              ...
              <aside className="rounded-[1.4rem] border border-[#dfddd2] bg-white p-5 shadow-[0_1px_0_rgba(18,61,50,0.04)]">
                ...
                <div className="flex items-center justify-between text-[#123d32]">
                  ...
                  <span className="text-[1.03rem] font-bold">Rp {totalPayment.toLocaleString('id-ID')}</span>
                </div>

                <button type="button" disabled={cartItems.length === 0} className="mt-5 flex w-full items-center justify-between rounded-[1rem] border border-[#dfddd2] bg-[#faf8f1] px-4 py-3 text-left text-[#123d32] transition hover:bg-[#f4f1e6] disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:bg-[#faf8f1]">
                  <div className="flex items-center gap-3">
                    <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#ecf5ef] text-[#123d32]">
                      <Coffee className="h-5 w-5" />
                    </span>
                    <div>
                      <p className="text-[0.9rem] font-semibold">Pilih metode pembayaran</p>
                      <p className="text-[0.72rem] text-[#7f887d]">Pilih cara pembayaran yang nyaman</p>
                    </div>
                  </div>
                  <ChevronRight className="h-4 w-4 text-[#7f887d]" />
                </button>

                <button type="button" disabled={cartItems.length === 0} className="mt-6 w-full rounded-full bg-[#0f4b37] px-5 py-3 text-[0.92rem] font-semibold text-white shadow-[0_8px_18px_rgba(15,75,55,0.18)] transition hover:bg-[#0b3f2d] disabled:cursor-not-allowed disabled:bg-[#9eb6ad] disabled:shadow-none">
                  Pesan Sekarang
                </button>
